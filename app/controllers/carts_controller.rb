class CartsController < ApplicationController
  def create
    if current_user.entity_type == "Customer"
      @cart = Cart.new(customer_id: current_user.id,restaurant_id: params[:id],order_status:0)
      if @cart.save!
  ...
  def update_cart_status
    ...
    if status && @cart && order[status] > order[@cart.order_status]
      ...
      if params[:order_status] == "order_accepted"
        @order =  Order.create(customer_id: current_user.id, restaurant_id: params[:id],cart_id: params[:cart_id],status:0)
        @order.save!
      end
      if @order
